# 🧬 Karcytics Distribution Registry
...
Welcome to the **Karcytics Distribution Registry**. This repository serves as the official decentralized index for discovering the **Karcytics Core Application** updates and its suite of advanced scientific analysis plugins. It also provides the root cryptographic authority for the core system.

---

## 📂 Core Registry Files

*   **[`registry.json`](registry.json)**: The official pointer index. In version 2, this file strictly maps plugin IDs to their source repositories (`repo_url`). The Karcytics Core application uses this file to eagerly fetch live plugin metadata (versions, descriptions, authors, etc.) directly from each plugin's `pyproject.toml`.
...
Karcytics relies on a decentralized, eagerly-loaded architecture for plugins. 
...
To maintain security and integrity, Karcytics validates all downloads using public-key cryptography.

| Authority Name | Authority ID | Public Key (SHA-256) |
| :--- | :--- | :--- |
| **Karcytics Core Authority** | `karcytics_core` | `08f4319b6f979057b36b0db2b8faaee6eff8782f3aafd5e924ba79b04d4c8366` |
